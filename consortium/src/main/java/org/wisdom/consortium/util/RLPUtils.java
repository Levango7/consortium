...
public class RLPUtils {
    ...
    public static void main(String[] args) {
        ...
        stringList.add("333");
        byte[] eee=new byte[0];
        Test test=new Test(1,1L,"lalala", (byte) 0x01,eee,stringList);

        byte[] a1=RLPUtils.encodeInt(test.getA());
        byte[] b1=RLPUtils.encodeElement(ByteUtil.longToBytes(test.getB()));
        byte[] c1=RLPUtils.encodeString(test.getC());
        byte[] d1=RLPUtils.encodeByte(test.getD());
        byte[] e=RLPUtils.encodeElement(test.getE());
        byte[][] f = new byte[test.getF().size()][];
        for(int x=0;x<test.getF().size();x++){
            f[x] = RLPUtils.encodeList(
                    RLPUtils.encodeString(test.getF().get(x)));
        }
        byte[] flist = RLPUtils.encodeList(f);
        byte[] encoded=RLPUtils.encodeList(a1,b1,c1,d1,e,flist);
    ...
    public static  class Test{
        ...
        public Test(int a, long b, String c, byte d, byte[] e, List<String> f) {
            this.a = a;
            this.b = b;
            this.c = c;
            this.d = d;
